...
def ms_test(g):    
    best_sol, best_cost = ls.multistart(g, 50)
...
def gr_test(g):
    best_sol, best_cost = ls.grasp(g, len(g)//5, 50)
...
def tests():
    for g_str in datasets:
        ...
        np.save("tests/simulated_annealing/sa_{}.npy".format(g_str),results)   
        
def load():
    x = np.load("tests/simulated_annealing/sa_{}.npy".format('G1000.02'))
